...
struct editorSyntax {
  ...
  char **keywords;  //Array of strings to be highlighted
...
int editorRowCxToRx(erow *row, int cx) {
    ...
    for (j = 0; j < cx; j++) {
        if (row->chars[j] == '\t')
            rx += (SCRIPTOR_TAB_STOP - 1) - (rx % SCRIPTOR_TAB_STOP); //Calculate number of columns to the next tab stop
        rx++;
    }
    return rx;
}
...
int editorRowRxToCx(erow *row, int rx) {
    ...
    for (cx = 0; cx < row->size; cx++) {
        if (row->chars[cx] == '\t')
            cur_rx += (SCRIPTOR_TAB_STOP - 1) - (cur_rx % SCRIPTOR_TAB_STOP);
        cur_rx++;
...
void editorUpdateRow(erow *row) {
    ...
    free(row->render);  //Clear render string
    row->render = malloc(row->size + tabs*(SCRIPTOR_TAB_STOP - 1) + 1); //Allocates space for current row

    for (j = 0; j < row->size; j++) {
        if (row->chars[j] == '\t') {    //Checks for any tab characters
            row->render[idx++] = ' ';
            while (idx % SCRIPTOR_TAB_STOP != 0) row->render[idx++] = ' ';
        }else{
            row->render[idx++] = row->chars[j]; //Copys string to render string
        }
    }
    row->render[idx] = '\0'; //Sets last character as the null character
    row->rsize = idx;   //Sets render size to index
...
void editorInsertChar(int c) {
    if (E.cy == E.numrows) {
        editorInsertRow(E.numrows, "", 0); //Appends new row if at end of file
    }
    editorRowInsertChar(&E.row[E.cy], E.cx, c);
    E.cx++;
}

void editorInsertNewline(void) {    //Handles the enter keypress
    if (E.cx == 0) {
